...
//)
public class ResourceDataAction extends BaseAction{
	...
	private PageView<Resource>pageView=new PageView<Resource>();	//这里必须要构造新对象，不然刚打开没有currentPage参数传递过来，如果不新建也行，第一次打开必须传递currentPage参数过来
	
	public PageView<Resource> getPageView() {
		return pageView;
	}

	public void setPageView(PageView<Resource> pageView) {
	...
	public String execute() throws Exception {
		...
		List<Resource> resources = pageView.getRecords();
		
		
		
		String[] excludes = {"roles","resources","resource","sortedChildResources"};
		
		jsonMap = new HashMap<String, Object>();//定义map 
		jsonMap.put("total", pageView.getTotalRecord());//total键 存放总记录数
		jsonMap.put("rows", resources);//rows键 存放每页记录 list 
